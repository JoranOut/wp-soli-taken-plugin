 ...
 *
 * Every "renders without PHP errors" assertion works by reading PHP diagnostics
 * out of the rendered document. That only happens when both `WP_DEBUG` and
 * `WP_DEBUG_DISPLAY` are enabled: `wp_debug_mode()` leaves `display_errors`
 * untouched when `WP_DEBUG` is false, and then no diagnostic of any severity —
 * not even a fatal — reaches the page, so those assertions pass unconditionally.
 *
